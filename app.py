...
EIGENFUNCTION_FILE = (
    "nhanes_first_eigenfunction.csv"
)

FPCA_SPLINE_LOOKUP_FILE = (
    "fpca_spline_lookup.csv"
)

OPTIONS_FILE = os.environ.get(
    "OPTIONS_FILE",
    "/data/options.json"
)
...
fpca_spline_lookup = pd.read_csv(
    FPCA_SPLINE_LOOKUP_FILE
)
...
def get_spline_basis(fpca_value):
    ...
    spline_1 = np.interp(
        fpca_value,
        fpca_spline_lookup["FPCA_score_1"],
        fpca_spline_lookup["spline_1"]
    )

    spline_2 = np.interp(
        fpca_value,
        fpca_spline_lookup["FPCA_score_1"],
        fpca_spline_lookup["spline_2"]
    )
...
def predict_weibull_aft_output(
    ...
    coef = {
        ...
        "race_ethnicityOther Race - Including Multi-Racial":
            -0.26372838,
        ...
        "educationSome college/AA":
            0.01665806,

        "marital_statusLiving with partner":
            0.17851851,
    ...
    predicted_median_years = (
        predicted_median_months / 12
    )

    predicted_median_age_at_death = (
        float(age) +
        predicted_median_years
    )
    ...
    survival_5yr = math.exp(
        -((60 / weibull_scale_param)
          ** weibull_shape)
    )

    survival_10yr = math.exp(
        -((120 / weibull_scale_param)
          ** weibull_shape)
    )

    survival_15yr = math.exp(
        -((180 / weibull_scale_param)
          ** weibull_shape)
    )
    ...
    return {
        ...
        "linear_predictor_log_months":
            float(lp),

        "predicted_median_survival_months":
            float(predicted_median_months),

        "predicted_median_survival_years":
            float(predicted_median_years),

        "predicted_median_age_at_death":
            float(predicted_median_age_at_death),

        "predicted_probability_of_dying_within_5_years":
            float(1 - survival_5yr),

        "predicted_probability_of_dying_within_10_years":
            float(1 - survival_10yr),

        "predicted_probability_of_dying_within_15_years":
            float(1 - survival_15yr),
        ...
        "weibull_scale_survreg":
            float(scale),

        "weibull_shape":
            float(weibull_shape),

        "active_categorical_coefficients":
            active_coefficients
    }
...
def health():
    ...
    return jsonify({
        "status": "ok",
        "message": "FPCA AFT backend running"
    })
