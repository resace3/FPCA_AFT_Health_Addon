...
def get_fpca_score_from_home_assistant_steps(
    ...
    df["last_changed_utc"] = pd.to_datetime(df["last_changed"], format="ISO8601", utc=True)

    df["last_changed_local"] = df["last_changed_utc"].dt.tz_convert(local_timezone)

    df["steps_cumulative"] = pd.to_numeric(df["state"], errors="coerce")

    bad_numeric_rows = df[df["steps_cumulative"].isna()].copy()

    df = (
        df.dropna(subset=["steps_cumulative"])
    ...
    df["steps_increment"] = np.where(df["step_diff"] < 0, df["steps_cumulative"], df["step_diff"])

    df["steps_increment"] = df["steps_increment"].fillna(0)

    df.loc[df["steps_increment"] < 0, "steps_increment"] = 0
    ...
    fitbit_week_curve = hourly["steps_increment"].to_numpy()

    if len(fitbit_week_curve) != 168:
        raise ValueError("Final Fitbit curve is not length 168")

    fitbit_centered = fitbit_week_curve - nhanes_mean_curve

    score_contribution = fitbit_centered * eigenfunction_1

    fpca_score_1 = float(np.sum(score_contribution))
    ...
    return {
        ...
        "hourly": hourly.assign(hour_local=hourly["hour_local"].astype(str)).to_dict(
            orient="records"
        ),
